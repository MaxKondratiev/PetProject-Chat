...
class PeopleViewController: UIViewController {
    ...
    //Уже в 34 уроке делаем инициализатор , чтобы передать от модели юзер username в title
    ...
    @objc func signOut(){
        let alertC = UIAlertController(title: nil, message: " Хотите выйти?", preferredStyle: .alert)
        let action = UIAlertAction(title: "Cancel", style: .cancel, handler: nil)
        alertC.addAction(action)
        alertC.addAction(UIAlertAction(title: "Sign out", style: .destructive, handler: { (_) in
            do {
